"""
Dev-UI Tool Demonstration
==========================

This script demonstrates how to use the Dev-UI tool for visualizing and debugging
Agent Framework workflows. The Dev-UI provides a powerful interface for:

Key Features:
-------------
1. **Workflow Visualization**: See the structure and flow of your workflow graph
2. **Real-time Execution**: Watch messages flow between executors in real-time
3. **State Inspection**: Examine the state of executors and workflow context
4. **Debugging Support**: Identify bottlenecks and issues in workflow execution
5. **Interactive Development**: Rapidly prototype and test workflow designs

What This Script Shows:
-----------------------
- Complex data processing workflow with multiple executors
- Various executor patterns (validation, transformation, enrichment, analysis)
- Rich data structures using Pydantic models
- Conditional routing based on validation results
- Structured output for different data types

How to Use Dev-UI:
------------------
1. Run this script
2. The Dev-UI will automatically open in your default browser
3. Observe the workflow execution in the visual interface
4. Explore the message flow and executor states

This example creates a realistic data processing pipeline with:
- Data validation
- Quality checks
...
The Dev-UI helps you understand how data flows through each stage and makes
debugging complex workflows much easier.
"""
...
class ProcessingRequest(BaseModel):
    ...
    enable_schema_validation: bool = Field(
        description="Enable schema validation checks", default=True
    )

    enable_security_validation: bool = Field(
        description="Enable security validation checks", default=True
    )

    enable_quality_validation: bool = Field(
        description="Enable data quality validation checks", default=True
    )
    ...
    description: str | None = Field(
        description="Optional description of the processing request", default=None
    )
...
class DataIngestion(Executor):
    ...
    async def ingest_data(
        self, request: ProcessingRequest, ctx: WorkflowContext[DataBatch]
    ) -> None:
...
class SchemaValidator(Executor):
    ...
    async def validate_schema(
        self, batch: DataBatch, ctx: WorkflowContext[ValidationReport]
    ) -> None:
...
class SecurityValidator(Executor):
    ...
    async def validate_security(
        ...
        issues = (
            1 if batch.data_type in [DataType.CUSTOMER, DataType.TRANSACTION] else 2
        )
...
class ValidationAggregator(Executor):
    ...
    async def aggregate_validations(
        ...
        should_fail = False  # has_errors or (quality_score < request.quality_threshold) NOTE: DEMO TO MAKE IT PASS
...
class DataNormalizer(Executor):
    ...
    async def normalize_data(
        self, batch: DataBatch, ctx: WorkflowContext[TransformationResult]
    ) -> None:
        ...
        success = (
            not request.force_transformation_failure
        )  # 75% success rate simplified to always success
...
class DataEnrichment(Executor):
    ...
    async def enrich_data(
        ...
        success = (
            not request.force_transformation_failure
        )  # 67% success rate simplified to always success
...
class DataAggregator(Executor):
    ...
    async def aggregate_data(
        ...
        success = (
            not request.force_transformation_failure
        )  # 80% success rate simplified to always success
...
class PerformanceAssessor(Executor):
    ...
    async def assess_performance(
        ...
        quality_score = (
            success_rate * 0.7 + (1 - min(avg_processing_time / 10, 1)) * 0.3
        ) * 100
...
def create_complex_workflow():
    ...
    return (
        ...
        .add_fan_out_edges(
            data_ingestion, [schema_validator, quality_validator, security_validator]
        )
        # Fan-in from validation to aggregator
        .add_fan_in_edges(
            [schema_validator, quality_validator, security_validator],
            validation_aggregator,
        )
        # Fan-out to transformation stage
        .add_fan_out_edges(
            validation_aggregator,
            [data_normalizer, data_enrichment, data_aggregator_exec],
        )
        ...
        .add_fan_in_edges(
            [data_normalizer, data_enrichment, data_aggregator_exec], accuracy_assessor
        )
